...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        # Connection of the socket to the port
        server_socket.bind((host, port))

        # The amount of requests that the server can handle at the same time
        ...
        while True:
            try:
                ...
                # Waiting to connection request from the client and creating the connection socket to the client
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            try:
                ...
                # Sending the bytes through the socket
                client_socket.send(response)

                # Closing the connection socket for specific client
                client_socket.close()
                break
