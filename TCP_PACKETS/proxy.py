...
def proxy(proxy_address: tuple[str, int], server_adress: tuple[str, int]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_socket:
        ...
        # Connection of the proxy socket to the port
        proxy_socket.bind((proxy_host, proxy_port))

        # The amount of requests that the proxy socket can handle at the same time
        ...
        while True:
            try:
                ...
                # Waiting to connection request from the client and creating the connection proxy socket to the client
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int], server_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            # Reading bytes from the client socket
            ...
            try:
                ...
                # Sending the bytes via the client socket
                client_socket.send(response)

                # Closing the connection client socket for specific client
                client_socket.close()
                break
